...
bet(player, args)
{
  ...
  self endon("death"); // ends on player death
	...
	{
		if(int(player.pers["rank"]) >= 5) // using a rank system
		{
		...
		{
			player tell("You need to be ^2level 5^7 to use the ^2.bet^7 commands!"); // tells player they are not of rank to use this command
		}
	}
}
